...
def Stemming(index):
    for TERM in list(index):
        ...
        if stemmed_term not in index:
            ...
            stem_freq, stemmedList = index[stemmed_term]
            intersected_list = IntersectionOfTwoLists(stemmedList, lst)
...
def RunningTheCompression():
    ...
    '''
    technique_list = [
        ('removing_numbers', DeletingNumbers),
    ...
    distance = '                         '
    header_term = '--------------------------------------------terms-------------------------------------------------------------'
    final_header = distance + header_term
    with open('Results/compression_table.txt', 'w', encoding="utf-8") as f:
        print(final_header, file=f)
